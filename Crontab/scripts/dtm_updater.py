...
def downloadRulesAndCheck(version:int, path:str='/tmp/dtm_rules.tgz'):
    """Download rules file and check md5 hash
    param: Rules version
    param: save file path
    """
    file_md5 = requests.get(f'{esm_srv_url}/esmapi/web/file/getmd5/dtm/{current_rules_version}/',
    headers=header).json().get('md5')
    response = requests.get(f'{esm_srv_url}/esmapi/web/file/download/dtm/{version}', headers=header, stream=True)
...
if __name__ == "__main__":
    ...
    if str2bool(os.getenv('DEV', 'False')):
        esm_srv_url= "https://test.api.esm.secbuzzer.co"
        print ("Running with DEV MODE")
    else:
        esm_srv_url= "https://api.esm.secbuzzer.co"
        print ("Running with Gold MODE")
    ...
    try:
        ...
        headers=header, json={'TypeCode': 'dtm'}).json().get('FileVersion')
    ...
    lv_path = "/tmp/local_dtm_rules_version" # local version file path
    if os.path.isfile(lv_path):
        with open(lv_path, 'r') as f:
            local_dtm_rules_version = f.readline().strip()
    else:
        local_dtm_rules_version = 0
    
    if current_rules_version != local_dtm_rules_version:
        ...
        for i in range(6): 
            ...
            if download_status:
                subprocess.call('mkdir -p /tmp/dtm', shell=True)
                subprocess.call('tar zxf /tmp/dtm_rules.tgz -C /tmp/dtm', shell=True)
                subprocess.call('chown 1000:1000 /tmp/* -R',shell=True)
                subprocess.call('rsync -a --delete /tmp/dtm /Suricata_rules/',shell=True)
                subprocess.call('rm -rf /tmp/it /tmp/it_rules.tgz',shell=True)
