...
curl -o it_rules.tgz "https://api.esm.secbuzzer.co/esmapi/web/file/download/it/$current_rules_version" -H "accept: */*" -H "authorization: $API_KEY"
tar zxvf it_rules.tgz -C tmp/it_rules
sudo chown 1000:1000 /tmp/* -R
sudo rsync -r --delete tmp/it_rules/ Suricata/suricata/rules/it/
rm -rf tmp it_rules.tgz
