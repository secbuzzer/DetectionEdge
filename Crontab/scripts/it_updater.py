...
def downloadRulesAndCheck(version:int, path:str='/tmp/it_rules.tgz'):
...
if __name__ == "__main__":
    ...
    lv_path = "/tmp/local_it_rules_version" # local version file path
    if os.path.isfile(lv_path):
        with open(lv_path, 'r') as f:
            local_it_rules_version = f.readline().strip()
    else:
        local_it_rules_version = 0
    
    if current_rules_version != local_it_rules_version:
        ...
        for i in range(6): 
            ...
            if download_status:
                subprocess.call('mkdir -p /tmp/it', shell=True)
                subprocess.call('tar zxf /tmp/it_rules.tgz -C /tmp/it', shell=True)
                subprocess.call('chown 1000:1000 /tmp/* -R',shell=True)
                subprocess.call('rsync -a --delete /tmp/it /Suricata_rules/',shell=True)
                ...
                subprocess.call('rm /tmp/local_it_rules_version 2>/dev/null',shell=True)
                subprocess.call(f'echo {current_rules_version} > /tmp/local_it_rules_version',shell=True)
                print('Done!')
                break
            else:
                continue
